...
const AddSourceType = ({
  setIsopenCreateSourceType,
  agencyId,
  afterUpdate,
}: {
  setIsopenCreateSourceType: (value: boolean) => void;
  agencyId: any;
  afterUpdate: () => void;
}) => {
  const id = agencyId;
  ...
  const onFinish = async (value: any) => {
    ...
    if (id) {
      ...
      response = await updateSourceType(queryParams, updateobj).finally(() =>
        setLoading(false)
      );
    } else {
      response = await createSourceType(updateobj).finally(() =>
        setLoading(false)
      );
    }
    ...
    if (response?.status === 201 || response?.status === 200) {
      if (id) {
        Successnotify("Source type updated successfully");
        setLoading(false);
        setIsopenCreateSourceType(false);
        afterUpdate();
      } else {
        Successnotify("Source type added successfully");
        setLoading(false);
        setIsopenCreateSourceType(false);
      }
    } else {
  ...
  const fetchSourceDetails = async () => {
    ...
    try {
      const response = await getSourceTypeById(queryParam).finally(() => {
        setLoading(false);
      });
  ...
  const fetchSourceTypeForm = async () => {
    ...
    if (columnRes?.status === 200 || columnRes?.status === 201) {
      ...
      setSourceTypeForm(form[3]?.fields);
    }
  };
  useEffect(() => {
    if (type_ === "all") {
    ...
    <div className="base-card">
      <div className="page-heading">
        <h1 className="heading-text">
          ...
          {id ? "Update Source Type" : "Add Source Type"}
        </h1>
        <BaseButton
          type="default"
          className="secondary"
